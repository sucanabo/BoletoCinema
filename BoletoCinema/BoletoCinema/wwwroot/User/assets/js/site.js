...
//proceed btn
function addSession() {
    var posObj = dataSeatPos;
    var selectedObj = seatSelected;
    var total = totalPrice;
    `<%Session["posObj"] = "` + posObj + `"; %>`;
    alert(`<%=Session["posObj"] %>`);
    console.log(posObj, selectedObj, total);
}
...
function renderSeat(row) {
    ...
    function createFontSeat(number, area, couple) {
        ...
        for (let j = 0; j < number; j++) {
            ...
            if (couple && couple == true) {
                seat_img.src = "../User/assets/images/movie/seat02-free.png";
                seat_type.className = "single-seat seat-free-two";
            }
            else {
    ...
    function createSeatLine(element, tag, couple) {
        ...
        if (couple && couple == true) {
            createFontSeat(2, seat_area, couple = true);
            createFontSeat(3, seat_area, couple = true);
            createFontSeat(2, seat_area, couple = true);
        }
        else {
    ...
    for (let r = row; r > 0; r--) {
        ...
        if (r > 1) {
            ...
            createSeatLine(seat_container_couple, seat_tag, couple = true);
        }
    ...
    var seatPos = function () {
        allSingleSeat.className = "single-seat";
        let j = row;
        let k = 0;
        for (let i = 0; i < allSingleSeat.length; i++) {
            let seatPos = document.createElement("span");
            let isCouple = allSingleSeat[i].classList.contains("seat-free-two") ? true : false;
            seatPos.className = "sit-num";
            let posTag = String.fromCharCode(65 + j - 1);
            if (isCouple) {
                seatPos.innerText = posTag + (k + 1) + " " + posTag + (k + 2);
            }
            else {
    ...
    allSingleSeat.forEach(item => {
        item.addEventListener("click", () => {
            ...
            if (img.classList.contains("booked")) {
                ...
                if (item.classList.contains("seat-freet-two")){
